...
def render_normal(font, text):
    ...
    surf_arr, bbs = crop_safe(pygame.surfarray.pixels_alpha(surf), rect_union, bbs, pad=35)
    surf_arr = surf_arr.swapaxes(0, 1)


    """from matplotlib import pyplot as plt
    plt.imshow(surf_arr, interpolation='nearest')
    plt.savefig(f'/media/monster/My Passport/work/DataGen/SRNet-DataGen-passport-RF/srnet_data/test/text_{text}.jpg')

    plt.imshow(surf_arr, interpolation='nearest')
    plt.savefig(f'/media/monster/My Passport/work/DataGen/SRNet-DataGen-passport-RF/srnet_data/test/bbs_{text}.jpg')"""
...
def perspective(img, rotate_angle, zoom, shear_angle, perspect, pad):
    ...
    H = H_rotate.dot(H_shear).dot(H_scale)
